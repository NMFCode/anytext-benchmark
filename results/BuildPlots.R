...
nicePlot <- function(p) grid.arrange(p)
...
for (file in files) {
  ...
  if (nrow(result[is.na(result$Params),]) > 0) {
    ...
    result$Job_Id <- trim(result$Params)
  }
  ...
  ggsaveNice(gsub("-measurements.csv", "-boxplot.pdf", file), benchmarkBoxplot)
  ggsaveNice(gsub("-measurements.csv", "-barplot.pdf", file), benchmarkBarplot)
